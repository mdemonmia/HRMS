...
Call log:
  - Expect "toHaveValue" with timeout 120000ms
  - waiting for locator('//select[@formcontrolname=\'userStatus\']')
    4 × locator resolved to <select formcontrolname="userStatus" aria-label="Default select example" _ngcontent-ng-cli-universal-c4177489038="" class="form-control form-select ng-untouched ng-valid ng-dirty">…</select>
      - unexpected value "0"
...
```ts
  ...
  168 |     })
  169 | 
  170 |     test('check edit blank password working or not',async({page})=>{
  ...
  181 |         await expect(blPass).toHaveValue('');
  182 |         console.log('password is blank.');
  183 |     })
  184 | 
  185 |     test.only('check edit status working or not',async({page})=>{
  186 |         const user = new UserMgtPage(page);
  187 |         await user.getEditBtn(
  188 |             data.view_row.view_row_name.name
  189 |         );
  190 | 
  191 |         await user.getEdituser(
  192 |             data.edit_user.edit_blank_status
  193 |         );
  194 |         await user.getupdateandConfirm();
  195 |         const blstatus = await page.locator("//select[@formcontrolname='userStatus']");
> 196 |         await expect(blstatus).toHaveValue('');
      |                                ^ Error: expect(locator).toHaveValue(expected) failed
  197 |         console.log('status is blank.');
  198 |     })
  199 |     
  200 | 
  201 | })
