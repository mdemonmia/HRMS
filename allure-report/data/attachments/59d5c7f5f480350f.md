...
Call log:
  ...
  - waiting for locator('#userEmail')
    237 × locator resolved to <input type="text" id="userEmail" name="userEmail" autocomplete="off" formcontrolname="userEmail" _ngcontent-ng-cli-universal-c4118926259="" class="form-control ng-invalid ng-touched is-invalid ng-dirty"/>
...
```ts
  ...
  103 |     })
  104 | 
  105 |     test.only('check email with invalid data',async({page})=>{
...
> 112 |         await expect(user.Email).toHaveClass(/ng-pristine/);
      |                                  ^ Error: expect(locator).toHaveClass(expected) failed
  113 |     })
  114 |     
  115 | })
