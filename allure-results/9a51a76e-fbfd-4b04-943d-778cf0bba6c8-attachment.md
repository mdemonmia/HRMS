...
Locator: locator('#userLName')
Expected pattern: /ng-invalid/
Received string:  "form-control ng-pristine ng-valid ng-touched"
Timeout: 120000ms

Call log:
  - Expect "toHaveClass" with timeout 120000ms
  - waiting for locator('#userLName')
    237 × locator resolved to <input type="text" id="userLName" name="userLName" formcontrolname="userLName" _ngcontent-ng-cli-universal-c4118926259="" class="form-control ng-pristine ng-valid ng-touched"/>
        - unexpected value "form-control ng-pristine ng-valid ng-touched"
...
```ts
  ...
  43 |         );
  44 |         await user.getSaveBtnandExceptdialog()
> 45 |         await expect(user.lastName).toHaveClass(/ng-invalid/);
     |                                     ^ Error: expect(locator).toHaveClass(expected) failed
  46 |     })
  47 | })
```