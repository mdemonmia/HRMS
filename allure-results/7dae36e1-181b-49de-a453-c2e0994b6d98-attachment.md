...
Call log:
  - waiting for locator('text=  Access to the path "/var/www/hrsite/wwwroot/vendorLog/vendor_logo_1234" is denied.') to be visible
...
```ts
  ...
  44 | 
  45 |     async getSaveBtn(){
  46 |         await this.saveBtn.scrollIntoViewIfNeeded();
  47 |         await this.saveBtn.click();
  48 |         const cmodal = await this.page.locator('text=Are you sure you want to submit this?');
  ...
  52 | 
  53 |         const errorMsg = await this.page.locator('text=  Access to the path "/var/www/hrsite/wwwroot/vendorLog/vendor_logo_1234" is denied.');
