...
Call log:
  - waiting for locator('#vndrAddDist')
    - locator resolved to <select required="" id="vndrAddDist" name="vndrAddDist" formcontrolname="vndrAddDist" _ngcontent-ng-cli-universal-c373924197="" class="form-control form-select ng-untouched ng-pristine ng-valid">…</select>
  - attempting select option action
    ...
    - retrying select option action
      - waiting 100ms
    114 × waiting for element to be visible and enabled
        - did not find some options
      - retrying select option action
        - waiting 500ms
...
> 35 |         if(vform.dist !=='')await this.District.selectOption(vform.dist.trim().toLowerCase());
     |                                                 ^ TimeoutError: locator.selectOption: Timeout 60000ms exceeded.
  ...
  39 |         if(vform.orgType !=='')await this.OganizationType.selectOption(vform.orgType.trim().toLowerCase());
