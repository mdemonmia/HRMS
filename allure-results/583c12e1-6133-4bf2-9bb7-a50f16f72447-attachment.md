...
Locator: locator('//select[@formcontrolname=\'userOrgDivisionId\']')
...
Call log:
  ...
  - waiting for locator('//select[@formcontrolname=\'userOrgDivisionId\']')
    235 × locator resolved to <select formcontrolname="userOrgDivisionId" aria-label="Default select example" _ngcontent-ng-cli-universal-c4118926259="" class="form-control form-select ng-untouched ng-pristine ng-valid">…</select>
...
```ts
  ...
  75 |     
  76 | 
  77 |     test.only('checked org division with invalid data',async({page})=>{
...
> 84 |         await expect(user.orgDiv).toHaveClass(/ng-invalid/);   
     |                                   ^ Error: expect(locator).toHaveClass(expected) failed
  85 |     })
  86 |     
  87 | })
