...
```ts
  ...
  26  |         this.saveBtn = page.locator("button[type='submit']");
...
> 58  |             if(!roleFound) throw new Error(`Role "${umgt.role}" not found`);
  ...
  146 |             expect(dialog.message()).toContain('Are you sure you want to save this?');
  147 |             await dialog.accept();
  148 |         });
  149 | 
  150 |         this.page.once('dialog', async (dialog) => {
  151 |             expect(dialog.message()).toContain('Data saved successfully');
  152 |             await dialog.accept();
  153 |         });
  154 | 
  155 |         await this.saveBtn.click();
  156 |     }
  157 | 
  158 | } 
```